...
import excel "$user/$graphs/estimates.xlsx", sheet("gps") firstrow clear

append using  "$user/$data/5strat.dta"

drop if time>34

* GRP PROB SOLV ITS ONLY
set more off
local counter = 0 
...
foreach x of local study1 {		
	...
	foreach i of local outcome1 {
		...
		local graph1 (rarea lb_gps_its ub_gps_its time, fcolor(dimgray) lcolor(black) lpattern(shortdash_dot)) `graph1'
		local graph2 (line y_gps_its time, lcolor(black) lpattern(shortdash_dot)) `graph2'
...
graph export "$user/$graphs/Grp_prb_its_ext.pdf", replace
